...
def get_align_transformation(rec_meshfile, gt_meshfile, trans_init = np.eye(4)):
    """
...
def get_align_transformation_pc(rec_meshfile_pc, gt_meshfile_pc, trans_init = np.eye(4)):
    """
    Get the transformation matrix to align the reconstructed mesh to the ground truth mesh.
    """
    o3d_rec_pc = o3d.geometry.PointCloud(points=rec_meshfile_pc)
    o3d_gt_pc = o3d.geometry.PointCloud(points=gt_meshfile_pc)
    # trans_init = np.eye(4)
    threshold = 0.1
    reg_p2p = o3d.pipelines.registration.registration_icp(
        o3d_rec_pc, o3d_gt_pc, threshold, trans_init,
...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=True):
    ...
    if align:
        ...
        # init transform from ATE eval
        rot = np.loadtxt('rot_tmp.txt')
        trans = np.loadtxt('trans_tmp.txt')
        trans_mat = np.eye(4)
        trans_mat[:3, :3] = rot
        trans_mat[:3, 3] = trans

        transformation = get_align_transformation(rec_meshfile, gt_meshfile, np.linalg.inv(trans_mat))
        mesh_rec = mesh_rec.apply_transform(transformation)
        # mesh_gt = mesh_gt.apply_transform(transformation)

    rec_pc = trimesh.sample.sample_surface(mesh_rec, 200000)
    print(np.max(rec_pc[0], axis = 0))
    print(np.min(rec_pc[0], axis = 0))
    rec_pc_tri = trimesh.PointCloud(vertices=rec_pc[0])

    gt_pc = trimesh.sample.sample_surface(mesh_gt, 200000)
    print(np.max(gt_pc[0], axis = 0))
    print(np.min(gt_pc[0], axis = 0))
    gt_pc_tri = trimesh.PointCloud(vertices=gt_pc[0])

    # trans2 = get_align_transformation_pc(o3d.utility.Vector3dVector(rec_pc[0]),o3d.utility.Vector3dVector(gt_pc[0]))
    # print(trans2)
